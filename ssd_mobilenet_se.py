...
from nets import excite_mobilenet
import os


class SSDMobileNetSEV2(nn.Module):
    ...
    def __init__(self, phase, size, extras, head, top_down, final_features, cfg):
        super(SSDMobileNetSEV2, self).__init__()
        self.phase = phase
        ...
        # SSD network
        self.backbone = nn.ModuleList(excite_mobilenet.MobileNetSEV2(num_classes=self.num_classes, width_mult=0.75).features)
...
def add_extras(cfg):
    layers = []
    block = excite_mobilenet.InvertedResidual_SE
...
def build_ssd_mobilenet_se(phase, cfg):
    size = cfg['min_dim']
    ...
    return SSDMobileNetSEV2(phase, size, extras_, head_, top_down_, final_features_, cfg)
